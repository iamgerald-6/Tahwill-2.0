...
const Contact = () => {
  ...
  const postMail = async (data: emailType) => {
    const res = await axios.post(apiRoutes.mail.sendMail, {
      email: data.email,
    });
    return res.data;
  };
    ...
    <div className="flex flex-col items-center gap-2 px-8  py-24">
      <div className="w-full flex flex-col items-center">
        <motion.h3
          ...
          className="text-6xl text-center bg-gradient-to-r from-primary to-primary-100 bg-clip-text text-transparent inline-block"
        >
          Stay Updated
        </motion.h3>
        <motion.p
      ...
      <div className="mt-5">
        <form onSubmit={handleSubmit(FormSubmit)}>
          ...
          <div className="w-full mt-5 flex flex-col items-center">
            <Button
              loading={isPending}
              className=" rounded-xl border border-primary text-primary px-10"
            >
              Send
            </Button>
          </div>
